...
WP-N06A adds optional read-only `check_rscad_model(..., initialization=None)` planning and supplied load-flow evidence; see [LOADFLOW_INITIALIZATION.md](LOADFLOW_INITIALIZATION.md). Enabled legacy Runtime initialization is refused before backend, rack, and grant access, while omitted or disabled initialization remains unchanged. The installed SDK takes frequency first, and LF must precede Compile. This adds no qualified live adapter or automatic compiled-artifact mutation exemption; supplied convergence and preconditions do not prove live integration or permission.
