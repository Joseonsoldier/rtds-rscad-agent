...
Rack qualification hardening retains exact version matching and default `expected_rscad_version: "2.7.3"`. Operators may explicitly select the observed API version string `"2.7"` for Compile/Runtime. This does not identify or qualify an installed patch release; other strings remain refused, and the separate structured-patch version contract is unchanged. Changing settings invalidates old policies and workflow bindings, so prepare fresh scoped evidence.

Compile now requires the saved starting rack to match the selected rack; it no longer changes that setting only in memory. Public live drivers require direct remote ownership checks, stopped/unmodified state and verified non-forced closure. Interrupted or uncertain native attempts block later workflows across that data directory. Existing MCP signatures, schemas, defaults, native-graph ambiguity checks, LF/timing refusals and model-evaluation no-rack host remain unchanged.

WP-N11 adds source-distribution evaluation scripts and a separate ten-task contract, with no production package API/dependency changes. Legacy `evals/tasks.json` and `tools/run_evals.py` remain compatible. All ten current tasks have executable bounded contracts; N05–08 remain synthetic/offline, while N03/N04/N10 additionally require an explicit private operator suite and its exact SHA-256. Existing core fixture creation and N01/N02/N09 contracts remain compatible. Native trace records add optional native cleanup, observed-call and artifact-integrity fields; native success requires them. The explicit model runner requires a fresh output directory and reviewed Windows/Codex CLI version. Instrumented MCP results are development evaluation records, not production schema changes. See [model evaluation](MODEL_EVALUATION.md).
...
WP-N06A adds optional `check_rscad_model` load-flow initialization planning and supplied-evidence inspection. Enabled legacy Runtime `loadflow_initialization` is now refused before backend/rack/grant access; omitted or disabled initialization is unchanged. The SDK compatibility correction uses frequency as the first argument and requires load-flow initialization before Compile. No live qualification, grant permission, or automatic compiled-artifact mutation follows from preconditions or supplied convergence.
