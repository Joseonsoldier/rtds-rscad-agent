...
WP-N10 adds the read-only CLI `lines list`, `lines inspect ABS_TLI --sha256 SHA`, and `lines preview ABS_REQUEST_JSON`; no MCP tool/signature changes. The strict source-bound request schema, positive scalar profile, exact numeric preview, byte limits and unsupported generation steps are documented in [LINE_AUTHORING.md](LINE_AUTHORING.md). Supported parsing and completed previews never establish native constants generation or Compile success.
