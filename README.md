...
The [native editing checkpoints](docs/NATIVE_EDITING.md) add existing flat Draft edits and source-derived new-case insertion/wiring or GROUP clipboard reconstruction, with explicit UUID mapping and protected publication. Reconstruction passes synthetic tests; the local new-case trial stopped because its legitimate temporary backing file was rejected. Explicit user-directed recovery subsequently verified save/close/disconnect and resolved that incident. A provenance-bound guard fix, live reconstruction, automatic selection and the Runtime closed loop remain unqualified. Default static behavior is preserved.
