...
Existing backend long-form CSV can be converted by `capture_rtds_results(request)` without a live call. `kind: power_metric` adds fourteen documented sampled metrics, including overshoot, coherent-window THD and a single-mode damping estimate. Each metric has explicit options and optional acceptance bounds with output units. General rise time, arbitrary integrals, implicit unit conversion and image-derived samples remain unsupported. Synthetic traces validate the software path only.
