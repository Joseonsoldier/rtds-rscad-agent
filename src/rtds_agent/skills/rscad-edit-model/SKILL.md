...
The optional `backend` defaults to `static`. Explicit `native` supports existing flat Draft parameter/location edits and the sole operation `{"op":"rebuild_draft","strategy":"insert"}` or `strategy:"clipboard"`. Reconstruction requires one subsystem, 1–500 globally unique component IDs, an empty saved Runtime and no opaque archive payloads. Insert supports flat models; clipboard supports GROUP/hierarchy only after saved membership and explicit UUID/context mapping pass. The operator policy must allow every component type (including GROUP); insertion also requires every stored parameter to be allowed. No policy is created. Check the preview's scope and SDK evidence. A reconstruction preview has no predicted candidate hash; its plan, source, definitions, companions, policy and adapter hashes bind apply. Three task-scoped local adapter/Compile trials passed, but public live apply and other target/version combinations remain unqualified; do not inherit their success for the current request.

New-case ownership requires a newly appearing empty temporary archive within the current API call, exact live identity and an empty Draft readback. Existing temporary files are not accepted. NAME insertion preserves the stored spelling even when native getters hide placeholders. GROUP child readback uses the exact source-observed group-local placement plus the saved UUID/context and structure comparison. A native save can resize an empty Runtime canvas: only that difference permits copying the exact source RTX bytes after confirmed close, with raw native archive evidence, unchanged native DFX and exact reopen. Other metadata differences or populated Runtime are refused.
