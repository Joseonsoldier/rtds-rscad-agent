...
The optional `backend` defaults to `static`. Explicit `native` supports existing flat Draft parameter/location edits only and can connect to the local app after a reviewed preview. Check the preview's supported scope and SDK evidence before applying. `auto` currently returns static preview only and refuses apply because operation-scoped native construction/Compile qualification is not installed. Never substitute static apply silently. The native adapter saves a new isolated file, closes, reopens that exact path, checks readbacks and full parsed semantics, then publishes. Inspect its durable journal on failure. Unknown identity or unverified cleanup blocks subsequent native apply; no retry or force close is available. GROUP membership is inspectable in IR/comparisons but grouped mutation is not yet supported. Compile and Runtime still use their separate existing permissions and workflows.
